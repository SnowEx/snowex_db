...
from snowex_db.interpretation import (
    convert_cardinal_to_degree, add_date_time_keys, standardize_depth,
    avg_from_multi_sample, get_InSar_flight_comment, manage_utm_zone
)


@pytest.mark.filterwarnings('ignore:Assuming')
@pytest.mark.parametrize(
    "card,expected", [('n', 0), ('S', 180), ('S/SW', 202.5), ('West', 270)]
)
...
@pytest.mark.parametrize(
    "data, in_tz, expected_date, expected_time", [
        (
                {'date/time': '2020-01-01-00:00'}, 'US/Mountain', this_day,
                this_time,
        ),
        (
                {'date/local_time': '2020-01-01-00:00'}, 'US/Mountain',
                this_day, this_time,
        ),
        (
                {'date': '2020-01-01', 'time': '00:00'}, 'US/Mountain',
                this_day, this_time,
        ),
        # Test converting of the UTC GPR format which assumes the input tz is UTC
        (
                {'utcyear': 2020, 'utcdoy': 1, 'utctod': '070000.00'}, 'UTC',
                this_day, this_time,
        ),
        # Test handling the milliseconds
        (
                {'utcyear': 2019, 'utcdoy': 35, 'utctod': 214317.222}, 'UTC',
                date(year=2019, month=2, day=4),
                time(
                    hour=21, minute=43, second=17, microsecond=222000,
                    tzinfo=pytz.UTC
                ),
        ),
        # Test parsing of the new GPR data time on NSIDC
        (
                {'date': '012820', 'time': '161549.557'}, 'UTC',
                date(year=2020, month=1, day=28),
                time(
                    hour=16, minute=15, second=49, microsecond=557000,
                    tzinfo=pytz.UTC
                ),
        ),
        (
                {'Date&Time': '1/27/2020 11:00'}, 'US/Mountain',
                date(year=2020, month=1, day=27),
                time(hour=18, tzinfo=pytz.utc),
        ),
        (
                {'date': '1/27/2020'}, 'US/Pacific',
                date(year=2020, month=1, day=27),
                time(hour=8, tzinfo=pytz.utc),
        ),
        (
                {'date/local standard time': '2019-12-20T13:00'}, 'US/Pacific',
                date(2019, 12, 20),
                time(hour=21, minute=0, tzinfo=pytz.utc),
        ),
        # Test CSU GPR time
        (
                {'date': '020620', 'time': 'nan'}, 'US/Mountain',
                date(2020, 2, 6), None,
        ),
        # Test Mala GPR time
        (
                {'date': '28-Jan-20', 'time': '16:07'}, 'UTC',
                date(2020, 1, 28), time(16, 7, tzinfo=pytz.utc),
        ),
...
@pytest.mark.parametrize(
    "depths, expected, desired_format, is_smp",
    [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                {'density_a': 180, 'density_b': 200, 'density_c': 'nan'},
                'density', 190.0,
        ),
        # Test all nans scenario
        (
                {
                    'dielectric_constant_a': 'nan',
                    'dielectric_constant_b': 'nan',
                }, 'dielectric', np.nan,
        ),
    ]
)
...
@pytest.mark.parametrize(
    'data_name, expected', [
        (
                'amplitude of pass 1',
                'Overpass Duration: 2020-01-01 10:00:00 - 2020-01-01 12:00:00 (UTC)',
        ),
        (
                'correlation',
...
def test_get_InSar_flight_comment(data_name, expected):
    """
    Test we can formulate a useful comment for the uavsar annotation file
    and a dataname
    """
    blank = '{} time of acquisition for pass {}'

    desc = {
        blank.format('start', '1'): {
            'value': pd.to_datetime('2020-01-01 10:00:00 UTC'),
        },
        blank.format('stop', '1'): {
            'value': pd.to_datetime('2020-01-01 12:00:00 UTC'),
        },
        blank.format('start', '2'): {
            'value': pd.to_datetime('2020-02-01 10:00:00 UTC'),
        },
        blank.format('stop', '2'): {
            'value': pd.to_datetime('2020-02-01 12:00:00 UTC'),
        },
    }
