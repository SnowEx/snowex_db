...
class TestUNMGPR(TableTestBase, WithUploadedFile):
    kwargs = {
        ...
        'instrument': 'gpr',
        'instrument_model': 'Mala 800 MHz GPR',
        'timezone': 'UTC',
        'name': 'UNM GPR Data',
    }
    ...
    @pytest.mark.usefixtures("uploaded_file")
    @pytest.mark.parametrize(
        "name", ["UNM GPR Data gpr Mala 800 MHz GPR"]
    )
    def test_point_observation(self, name):
        records = self.get_records(PointObservation, "name", name)
        dates = [
            date(2020, 1, 28), date(2020, 1, 29), date(2020, 1, 30), date(2020, 1, 31),
        ]
    ...
    @pytest.mark.parametrize(
        "table, attribute, lon, lat", [
            (PointData, "geom", -108.1340183, 39.0296597),
        ]
    )
