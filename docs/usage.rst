...
Each table has a class already built in the snowex_db. At a minimum you need at
least one of those classes to interact with it using this library. To grab
all points in the table::

    from snowex_db.data import PointData, LayerData, ImageData, SiteData
...
3. Calling a few directly from `snowex_db.functions`
...
convenient to just make them for ourselves which is what is in snowex_db.
